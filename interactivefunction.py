...
class InteractiveFunction(InteractiveGraph):
    ...
    def update(self, proj: ndarray | None = None, dep_proj = None):
        ...
        dep_ellipses = self.DEPCALC.get_proj_ellipses(self.curr_dep_proj, [self.CONFS[m] for m in self.CONFS if self.m_dists_using[m]])
    ...
    def build_widgets(self):
        ...
        self.m_checkbox = CheckButtons(
            ax = self.axcheckbox,
            labels = list(self.CONFS.keys())[:len(self.CONFS)//2],
            label_props={'color': self.Palette.ellipse_colours[:len(self.CONFS)//2], "size":[14]*(len(self.CONFS)//2), "family":['serif']*(len(self.CONFS)//2)},
            frame_props={'edgecolor': self.Palette.ellipse_colours[:len(self.CONFS)//2], "facecolor":'white'},
            check_props={'facecolor': self.Palette.ellipse_colours[:len(self.CONFS)//2]},
            actives=list(self.m_dists_using.values())[:len(self.CONFS)//2]
        )
        self.m_checkbox.on_clicked(self.show_ellipse)
        self.m_checkbox2 = CheckButtons(
            ax = self.axcheckbox2,
            labels = list(self.CONFS.keys())[len(self.CONFS)//2:],
            label_props={'color': self.Palette.ellipse_colours[len(self.CONFS)//2:], "size":[14]*(len(self.CONFS)-len(self.CONFS)//2), "family":['serif']*(len(self.CONFS) - len(self.CONFS)//2)},
            frame_props={'edgecolor': self.Palette.ellipse_colours[len(self.CONFS)//2:], "facecolor":'white'},
            check_props={'facecolor': self.Palette.ellipse_colours[len(self.CONFS)//2:]},
            actives=list(self.m_dists_using.values())[len(self.CONFS)//2:]
        )
        self.m_checkbox2.on_clicked(self.show_ellipse)
